...
1. Scan GEO, capture images.
2. Detect satellites.
3. Check for dynamic tasks assigned.
4. Record live coordinates of ROO in a JSON file.
"""
...
def process_json_file(file_name, folder_path, scheduled_observations):
    try:
        ...
        with open(json_file_path, 'r') as json_file:
            ...
            elif 'az' in data and 'el' in data:
                target_az = data['az']
                target_el = data['el']

            if target_time is not None:
                target_datetime = datetime.datetime.strptime(target_time, '%Y-%m-%dT%H:%M:%S')
                slewing_time = target_datetime - datetime.timedelta(minutes=3)
                ...
                if current_time < slewing_time:
                    if len(scheduled_observations) == 0:
                        if 'ra' in data:
                            scheduled_observations.append((meas_type, target_datetime, target_ra, target_dec, exposure_time, sidereal_tracking, file_name))
                        elif 'az' in data:
                            scheduled_observations.append((meas_type, target_datetime, target_az, target_el, exposure_time, sidereal_tracking, file_name))
                    else:
                        if scheduled_observations[-1][5] != file_name:
                            if 'ra' in data:
                                scheduled_observations.append((meas_type, target_datetime, target_ra, target_dec, exposure_time, sidereal_tracking, file_name))
                                print(f"Scheduled observation for RA: {target_ra}, Dec: {target_dec} at {target_datetime}")
                            elif 'az' in data:
                                scheduled_observations.append((meas_type, target_datetime, target_az, target_el, exposure_time, sidereal_tracking, file_name))
                                print(f"Scheduled observation for Az: {target_az}, El: {target_el} at {target_datetime}")
    except Exception as e:
...
def check_shutter_status_and_record_observation(teleobj, domeObj, camObj, observation, folder_path):
    try:
        ...
        if observation[0] == "radec":
            meas_type, target_datetime, target_ra, target_dec, exposure_time, sidereal_tracking, file_name = observation
        elif observation[0] == "azel":
            meas_type, target_datetime, target_az, target_el, exposure_time, sidereal_tracking, file_name = observation

        # Calculate slewing time 5 minutes before the specified time
        slewing_time = target_datetime - datetime.timedelta(minutes=3)
        t_str = str(target_datetime).replace(':', '_')
        ...
        if stat != 0:
            ...
            observation_file_path = os.path.join(folder_path, f"unsafe_shutter_{t_str}")
            ...
            if observation[0] == "radec":
                ...
                teleobj.SlewToRaDec(target_ra, target_dec, f'Ra_{target_ra}_Dec_{target_dec}')
                while teleobj.IsSlewComplete != 1:
                    pass
                print("Telescope is in position.")
            elif observation[0] == "azel":
                print(f"Slewing to Az: {target_az}, El: {target_el} at {slewing_time}")
                teleobj.SlewToAzAlt(target_az, target_el, f'Az_{target_az}_El_{target_el}')
                while teleobj.IsSlewComplete != 1:
                    pass
                print("Telescope is in position.")
            ...
            if exposure_time <= 5.0:
                ...
                for i in range(5):
                    ...
                    observation_file_path = os.path.join(folder_path, f"image_captured_{t_str}_image_{i+1}.json")
                    with open(observation_file_path, 'w') as observation_file:
                        json.dump(observation_data, observation_file, indent=4)
                ...
                observation_file_path = os.path.join(folder_path, f"exposure_time_exceeded_{t_str}")
                with open(observation_file_path, 'w') as observation_file:
                    json.dump(observation_data, observation_file, indent=4)
    ...
    for observation in scheduled_observations:
        ...
        if current_time >= slewing_time:
            if observation[0] == "radec":
                ...
                print(f"Executing observation for Az: {target_az}, El: {target_el} at {current_time}")
